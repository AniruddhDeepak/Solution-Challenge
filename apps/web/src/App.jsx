  ...
  return (
    <div className="min-h-screen bg-white text-gray-800 flex font-sans overflow-hidden">
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden bg-gray-50/50 relative">
        ...
        {/* Premium Floating Watermarks (Supply Chain Icons) */}
                          ...
                          <motion.div key={group.category} variants={popIn}>
                            ...
                            <div className={`flex items-center justify-between mb-8 p-6 rounded-3xl ${cfg.bg} border-2 border-gray-900 shadow-[4px_4px_0px_0px_rgba(17,24,39,0.1)]`}>
                              <div className="flex items-center">
                                ...
                                <div>
                                  <h3 className="text-2xl font-black text-gray-900 tracking-tight">{group.category}</h3>
                                  <p className="text-sm font-bold text-gray-600 mt-1">{filteredItems.length} product{filteredItems.length !== 1 ? 's' : ''} • {totalUnits.toLocaleString()} total units</p>
                                </div>
                              </div>
                            </div>
                              ...
                              {filteredItems.map((item) => (
                                ...
                                >
                                  <div className="flex justify-between items-start mb-4 relative z-10">
                                    <div>
                                      <div className={`w-12 h-12 ${cfg.bg} border border-gray-200 rounded-2xl flex items-center justify-center mb-4 group-hover:scale-110 transition-transform shadow-sm`}>
                                        <CatIcon className={`w-6 h-6 ${cfg.color}`} />
                                      </div>
                                      <h3 className="text-2xl font-black text-gray-900 leading-tight mb-2">{item.name}</h3>
                                    </div>
                                    <div className="flex flex-col items-end gap-3">
                                      <span className={`inline-flex items-center px-3 py-1.5 rounded-lg text-xs font-black uppercase tracking-wider border-2 ${item.status === 'In Stock' ? 'bg-green-50 text-green-700 border-green-200' :
                                      ...
                                      <button
                                        ...
                                        className="p-2 bg-red-50 hover:bg-red-100 border border-red-100 hover:border-red-300 text-red-500 rounded-xl transition-all shadow-sm hover:shadow"
                                      >
                                        <Trash2 className="w-4 h-4" />
                                      </button>
                                    </div>
                                  </div>
                                  <div className="relative z-10 mt-auto pt-4 border-t-2 border-gray-50">
                                    <div className="flex justify-between items-end">
                                      <div>
                                        <p className="text-sm font-bold text-gray-400 mb-1">ID: {item.id.slice(0, 8)}...</p>
                                        <p className="text-sm font-bold text-gray-600 flex items-center"><Map className="w-4 h-4 mr-1.5 text-gray-400" /> {item.location}</p>
                                      </div>
                                      <div className="text-right">
                                        <span className="text-xs font-black uppercase tracking-widest text-gray-400 block mb-1">Quantity</span>
                                        <span className="text-5xl font-black text-emerald-600 drop-shadow-sm">{Number(item.count).toLocaleString()}</span>
                                      </div>
                                    </div>
                                  </div>
                                </motion.div>
                          ...
                          <motion.div key={wh.id} variants={popIn} whileHover={{ y: -4 }} className="bg-white p-6 rounded-3xl border-2 border-gray-900 shadow-[4px_4px_0px_0px_rgba(17,24,39,0.1)] hover:shadow-[6px_6px_0px_0px_rgba(17,24,39,0.15)] transition-all flex flex-col justify-between group">
                            <div className="flex justify-between items-start mb-6">
                              <div className="flex items-center gap-4">
                                ...
                                <div>
                                  <h3 className="text-xl font-black text-gray-900 leading-tight mb-1">{wh.name}</h3>
                                  <p className="text-sm font-bold text-gray-500 flex items-center"><Globe className="w-4 h-4 mr-1.5" />{wh.city || wh.region}</p>
                                </div>
                              </div>
                              <button onClick={() => deleteWarehouse(wh.id)} className="p-2 bg-red-50 hover:bg-red-100 border border-red-100 hover:border-red-300 text-red-500 rounded-xl transition-all shadow-sm hover:shadow"><Trash2 className="w-4 h-4" /></button>
                            </div>
                            <div className="pt-4 border-t-2 border-gray-50 flex justify-between items-center">
                              <span className="text-sm font-bold uppercase tracking-widest text-gray-400">Max Capacity</span>
                              <span className="text-3xl font-black text-blue-600 drop-shadow-sm">{Number(wh.capacity).toLocaleString()}</span>
                            </div>
                          </motion.div>
                        ))}
                      </div>
                          ...
                          <div>
                            <div className="flex justify-between items-start mb-2">
                              ...
                              <div className="flex items-center gap-1.5">
                                <span className="text-xs font-black text-gray-400 bg-gray-100 px-2 py-0.5 rounded-md border border-gray-200">QTY: {s.quantity}</span>
                            ...
                            <p className="text-sm font-bold text-gray-600 mb-4">{s.origin} → {s.destination}</p>
                            <button onClick={() => updateShipmentStatus(s.id, 'transit')} className="w-full py-3 bg-blue-50 hover:bg-blue-100 text-blue-700 font-bold rounded-xl text-sm transition-all border border-blue-200">Dispatch Shipment</button>
                          </div>
                        </motion.div>
                      ))}
                    </div>
                            ...
                            <div>
                              <div className="flex justify-between items-start mb-2">
                                ...
                                <div className="flex flex-col items-end gap-1">
                                  ...
                                  {isDelayed && <span className="text-[10px] font-black text-red-600 bg-red-100 px-2 py-0.5 rounded-md flex items-center border border-red-200"><TriangleAlert className="w-3 h-3 mr-1" />DELAYED</span>}
                                </div>
                              </div>
                              <p className="text-sm font-bold text-gray-600 mb-2">{s.origin} → {s.destination}</p>
                              {s.etaDate && <p className="text-xs font-bold text-gray-400 mb-3 flex items-center"><CalendarClock className="w-4 h-4 mr-1.5 text-gray-400" />ETA: {new Date(s.etaDate).toLocaleDateString()}</p>}
                              {s.transportMode && <p className="text-[10px] font-bold text-gray-400 mb-4 uppercase tracking-wider">{s.transportMode === 'truck' ? '🚛' : s.transportMode === 'air' ? '✈️' : s.transportMode === 'sea' ? '🚢' : '🚂'} {s.transportMode}</p>}
                              <button onClick={() => updateShipmentStatus(s.id, 'delivered')} className="w-full py-3 bg-emerald-50 hover:bg-emerald-100 text-emerald-700 font-bold rounded-xl text-sm transition-all border border-emerald-200">Mark Delivered</button>
                            </div>
                          </motion.div>
                        );
                      })}
                      ...
                      {shipments.filter(s => s.status === 'delivered').map(s => (
                        <motion.div key={s.id} layoutId={s.id} className="bg-white p-6 rounded-2xl border-2 border-gray-900 shadow-[4px_4px_0px_0px_rgba(17,24,39,0.1)] hover:shadow-[6px_6px_0px_0px_rgba(17,24,39,0.15)] transition-all opacity-80">
                          <div>
                            ...
                            <p className="text-sm font-bold text-gray-600 mb-4">{s.origin} → {s.destination}</p>
                            <div className="w-full py-3 text-emerald-600 font-black text-sm text-center flex items-center justify-center border-t-2 border-emerald-50 pt-4"><CheckCircle2 className="w-5 h-5 mr-1.5" /> Delivery Confirmed</div>
                          </div>
                        </motion.div>
                      ))}
                    </div>
